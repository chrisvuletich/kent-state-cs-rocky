...
function loadInitialFrame(): FrameName {
	...
	try {
		if (new URLSearchParams(window.location.search).get('doc')?.trim()) {
			return 'help';
		}
		const requestedFrame = new URLSearchParams(window.location.search).get('frame')?.trim();
		if (isFrameName(requestedFrame)) {
