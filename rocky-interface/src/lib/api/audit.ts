...
type ApiAuditLog = Partial<{ u_id: string; user_name: string; user_email: string; user_role: 'admin' | 'student'; c_id: string; event_type: string; created: string }>;
...
export async function fetchAuditLogs(filters: Record<string, string> = {}): Promise<AuditLog[]> {
	...
	return rows.map((row) => ({
		...
		userRole: row.user_role === 'admin' ? 'admin' : 'student', course: row.c_id?.trim() || 'No course', action: row.event_type?.trim() || 'unknown', created: row.created?.trim() || ''
	}));
}
