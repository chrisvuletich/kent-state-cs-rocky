<script lang="ts">
	...
	$: requestPath = linePath(
		buckets.map((bucket) => bucket.requests),
		maxRequests
	);
	$: tokenPath = linePath(
		buckets.map((bucket) => bucket.usage.total_tokens),
		maxTokens
	);
	...
	function linePath(values: number[], maximum: number): string {
		return values
			.map(
				(value, index) =>
					`${index === 0 ? 'M' : 'L'} ${x(index, values.length).toFixed(2)} ${y(value, maximum).toFixed(2)}`
			)
			.join(' ');
	}
	...
	<div class="throughput-chart" bind:this={container}>
		...
		{:else}
			<svg
				...
				role="img"
				aria-label={`Requests and tokens over ${windowLabel.toLowerCase()}`}
			>
				{#each [0, 0.25, 0.5, 0.75, 1] as ratio}
					...
					></line>
					<text
						class="axis-label request-axis"
						x={margin.left - 10}
						y={margin.top + innerHeight * ratio + 4}
						text-anchor="end">{compact(maxRequests * (1 - ratio))}</text
					>
					<text
						class="axis-label token-axis"
						x={chartWidth - margin.right + 10}
						y={margin.top + innerHeight * ratio + 4}>{compact(maxTokens * (1 - ratio))}</text
					>
				{/each}
				<text class="axis-title request-axis" x={margin.left} y={12}>Requests</text>
				<text class="axis-title token-axis" x={chartWidth - margin.right} y={12} text-anchor="end"
					>Tokens</text
				>
				<line
				...
				{#each xTicks as index}
					<text
						...
						>{timeLabel(buckets[index].start)}</text
					>
				{/each}
...
<style>
	...
	.throughput-heading h2 {
		margin: 0 0 2px;
		font-size: var(--font-size-base);
		color: var(--color-text-primary);
	}
	.throughput-heading p,
	figcaption {
		margin: 0;
	...
	.token-path {
		stroke: var(--color-highlight-gold);
	}
	figcaption {
