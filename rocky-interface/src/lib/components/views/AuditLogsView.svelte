<script lang="ts">
	...
	async function loadLogs(): Promise<void> {
		loading = true; error = '';
		try { logs = await fetchAuditLogs({ search, role, course, action, date_from: dateFrom, date_to: dateTo }); }
		catch (err) { error = err instanceof Error ? err.message : 'Unable to load audit logs.'; }
		finally { loading = false; }
	}
	function clearFilters(): void { search = ''; role = ''; course = ''; action = ''; dateFrom = ''; dateTo = ''; loadLogs(); }
...
<ViewShell title="Audit Logs">
	<section class="section audit-log-view">
		...
		<form class="audit-filters" onsubmit={(event) => { event.preventDefault(); loadLogs(); }}>
			...
			<button class="view-btn" type="submit">Apply filters</button><button class="view-btn" type="button" onclick={clearFilters}>Clear</button>
		</form>
